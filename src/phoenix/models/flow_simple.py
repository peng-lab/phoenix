...
class RMSNorm(nn.Module):
    ...
    ----------
    normalized_shape
        Shape of the trailing dimensions to normalize over.
    eps
        Value added to the denominator for numerical stability.
    elementwise_affine
        Whether to apply a learned per-element scale after normalizing.
    """
    ...
    def forward(self, x: Tensor) -> Tensor:
        """
        Apply RMS normalization (and, if enabled, a learned scale) to `x`.

        Parameters
        ----------
        x
            Input tensor whose trailing dimensions match `normalized_shape`.
...
class SwiGLUFFN(nn.Module):
    ...
    def __init__(
        ...
        d_hidden = d_hidden or d_input

        if d_output is None:
            d_output = d_input

        self.swiglu = nn.Module()
        self.swiglu.w1 = nn.Linear(
            d_input,
            d_hidden,
            bias=ffn_bias,
        )
        self.swiglu.w2 = nn.Linear(
            d_input,
            d_hidden,
            bias=ffn_bias,
        )
        self.swiglu.w3 = nn.Linear(
            d_hidden,
            d_output,
            bias=ffn_bias,
        )
        # register as a proper sub-module
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        """
        gate = F.silu(self.swiglu.w1(x))
        value = self.swiglu.w2(x)
        return self.swiglu.w3(gate * value)
...
class FlashAttention(nn.Module):
    ...
    def forward(self, x: Tensor, c: Tensor | None = None):
        ...
        xo = xo.transpose(1, 2).contiguous().view(x_bsz, x_sql, -1)
